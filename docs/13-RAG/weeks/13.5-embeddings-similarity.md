...
![Embeddings &amp; similarity — diagram](../diagrams/13.5-embeddings-similarity-hero.svg)
...
> 🖼️ **Image placeholder — Embedding space (2D projection).** A t-SNE/UMAP scatter of a few hundred sentence embeddings, colored by topic, showing that semantically similar sentences form tight clusters while unrelated ones sit far apart — the geometric intuition behind cosine retrieval.
> _Suggested asset:_ `diagrams/embedding-space-scatter.png` _(a real illustration to add; the Mermaid/SVG diagrams above cover the schematic view)._
